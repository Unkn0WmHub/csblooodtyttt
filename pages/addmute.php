        ...
        <div class="card_header">
            <div class="svg_text_header">
                ...
                <div class="header_text">
                    <div class="flex_header_top"><?= $Translate->get_translate_module_phrase('module_page_minisystem', '_ASAddMute') ?></div>
                    <div class="flex_header_bottom"><?= $Translate->get_translate_module_phrase('module_page_minisystem', '_ASAddMuteDesc') ?></div>
                </div>
            </div>
        </div>
        <div class="grid_control">
            <div class="card">
                ...
                <div class="flex_over_adm scroll">
                    <?php foreach ($AdminSystemInfoMute as $mute) : ?>
                        <div class="card_lists_nakaz">
                            <?php if (!empty($General->arr_general['avatars'])) : ?>
                                <span class="info_card_table_head_item"><img class="admin_avatar" id="<?php $General->arr_general['avatars'] === 1 && print con_steam32to64($mute['steamid']) ?>" <?= $sz_i < '20' ? 'src' : 'data-src' ?>="<?= $General->getAvatar(con_steam32to64($mute['steamid']), 2) ?>" title="" alt=""></span>
                            <?php endif; ?>
                            <span class="info_card_table_head_item"><?= date('d.m.Y',$mute['created']) ?></span>
                            <span class="info_card_table_head_item"><?= action_text_clear(action_text_trim($mute['name'], 13)) ?></span>
                            <span class="info_card_table_head_item"><?= action_text_clear(action_text_trim($mute['admin_name'], 13)) ?></span>
                            <span class="info_card_table_head_item"><?= action_text_clear(action_text_trim($mute['reason'], 13)) ?></span>
                            <span class="info_card_table_head_item"><?php $createdTime = $mute['created']; $duration = $mute['duration']; if ($duration == 0) { echo $Translate->get_translate_phrase('_Forever'); } else { if (time() > $createdTime + $duration) { echo $Translate->get_translate_module_phrase('module_page_minisystem', '_ASExpLox'); } else { echo $Modules->action_time_exchange($duration); } } ?></span>
                            <div class="flex_del">
                                <a href="<?= $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64($mute['steamid']) : $mute['steamid'] ?>" target="_blank" class="btn_dev" data-tippy-content="<?= $Translate->get_translate_module_phrase('module_page_minisystem', '_ASProf') ?>" data-tippy-placement="top">
                    ...
                    <form id="add_mute_as">
                        <div class="input-container">
                            ...
                            <div class="input-wrapper">
                                ...
                                <input type="text" name="steam_player_mute" placeholder="STEAM_1:1:390... / 7656119803... / [U:1:1234234] / https://steamcommunity.com/profiles/..." style="margin-left: 42px;" required>
                            </div>
                        </div>
                        ...
                        <?php if($MiniSystem->GetSettings['time_choice'] == 0) : ?>
                            <div class="input-container">
                                ...
                                <div class="input-wrapper">
                                    ...
                                    <input type="number" name="duration_mute" placeholder="0" style="margin-left: 42px;">
                                </div>
                            </div>
                        <?php endif; ?>
                        <div class="input-container">
                            ...
                            <div class="input-wrapper">
                                ...
                                <input type="text" name="reason_mute" placeholder="<?= $Translate->get_translate_module_phrase('module_page_minisystem', '_ASVorRes') ?>" style="margin-left: 42px;" required>
                            </div>
                        </div>
                    </div>
                    <input class="btn w100 btn5px" type="submit" value="<?= $Translate->get_translate_module_phrase('module_page_minisystem', '_ASAddMute') ?>">
                </form>
            </div>
        </div>
        <?php if ($page_max_mute != 1) : ?>
            <div class="pagination">
                ...
                <?php for ($v = $page_num; $v < min($page_num + 3, $page_max_mute); $v++) { ?>
                    <a href="<?= set_url_section(get_url(2), 'num', $v + 1) ?>" class="button_pagination current "><?= $v + 1 ?></a>
                <?php } ?>
                <?php if ($page_num != $page_max_mute) { ?>
                ...
                <?php } ?>
                <?php if ($page_num = $page_max_mute) { ?>
                    <a href="<?= set_url_section(get_url(2), 'num', $page_max_mute) ?>" class="button_pagination current " data-tippy-content="<?= $Translate->get_translate_phrase('_IntheEnd') ?>" data-tippy-placement="bottom">
                        <svg viewBox="0 0 448 512">
